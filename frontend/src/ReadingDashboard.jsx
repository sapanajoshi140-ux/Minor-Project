    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-neutral-800 border border-white/10 rounded-2xl p-6 w-80">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white/60 hover:bg-white/10 transition">
            Cancel
          </button>
          <button
            onClick={() => onSave(goal)}
...
const ReadingDashboard = ({
  ...
  const mergeTimeMap = useCallback((timeMap) => {
    ...
    setDocuments(prev => prev.map(doc => {
      ...
      return timeMap[id] != null ? { ...doc, time_spent_seconds: timeMap[id] } : doc;
    }));
  }, []);
  ...
  const fetchAllDocuments = useCallback(async () => {
    ...
    while (hasMore) {
      try {
        const res = await fetch(`${apiUrl}/documents?page=${page}&limit=100`, { headers: getHeaders() });
        if (!res.ok) break;
  ...
  const updateReadingGoal = async (newGoal) => {
    ...
    try {
      ...
      if (mountedRef.current) { setReadingGoal(newGoal); setShowGoalModal(false); }
      await silentRefresh();
    } catch (err) {
  ...
  const filteredDocsTab = documents.filter(d =>
  ...
  const UploadCard = (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
      ...
      <UploadSection onUpload={handleUpload} isProcessing={isUploading} error={uploadError} />
    </div>
  );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div className="bg-neutral-800 border border-white/10 rounded-2xl p-6 w-96">
          ...
          <p className="text-sm font-medium text-white mb-4 truncate">{doc.filename}</p>
          {deleteError && (
